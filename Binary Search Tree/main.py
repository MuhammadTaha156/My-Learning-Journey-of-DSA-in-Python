...
class BST:
    ...
    def r_search(self,root,x):
        if root is None or root.item==x:
            return root
        if x<root.item:
    ...
    def deletion(self,x):
        self.root=self.r_deletion(self.root,x)

    def r_deletion(self,root,x):
        if root is None:
            return root
        if x<root.item:
            root.left=self.r_deletion(root.left,x)
        elif x>root.item:
            root.right=self.r_deletion(root.right,x)
        else:
